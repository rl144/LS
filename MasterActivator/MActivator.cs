...
namespace MasterActivator
{
    internal class MActivator
    {
                ...
                if (Config.Item("enabled").GetValue<KeyBind>().Active)
                {
                    ...
                    if (attacker.Type == GameObjectType.obj_AI_Hero && attacker.IsEnemy)
                    {
                        if (args.SData.Name == akaliW.name)
                        {
                            ...
                            {
                                if ((Config.Item("akaliWOnCombo").GetValue<bool>() && Config.Item("comboModeActive").GetValue<bool>()) ||
        ...
        private void onGameUpdate(EventArgs args)
        {
            if (Config.Item("enabled").GetValue<KeyBind>().Active)
            {
                try
                {
                    ...
                    if (Config.Item("comboModeActive").GetValue<bool>())
                    {
                        combo();
                    }
                }
                            ...
                            if (Items.CanUseItem(item.id))
                            {
                                ...
                                if (activeAllyHeros.Count() > 0)
                                {
                                    foreach (Obj_AI_Hero hero in activeAllyHeros)
                                    {
                                        ...
                                        {
                                            if ((Config.Item("defJustOnCombo").GetValue<bool>() && Config.Item("comboModeActive").GetValue<bool>()) ||
        ...
        private void createMenu()
        {
            ...
            Config.SubMenu("combo").AddItem(new MenuItem("comboModeActive", "Active")).SetValue(true);
